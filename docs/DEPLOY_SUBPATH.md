...
目标地址：**https://reiinoki.dpdns.org/md-altas/**。
`https://reiinoki.dpdns.org/` 和其他路径仍由原网站提供；不要把整个域名改绑到 MD Atlas。

## 发布结构

`site.config.js` 定义基础路径 `/md-altas/`。Vite 仍构建到 `dist/`，用于普通本地预览；
随后 `scripts/prepare-workers-assets.mjs` 只复制构建输出，生成 Cloudflare 的独立静态发布包：

```text
.wrangler/assets/
  _headers
  index.html                 # SPA 回退入口，引用 /md-altas/ 下的资源
  md-altas/
    index.html
    favicon.svg
...
1. DNS zone 与 `md-altas` Worker 位于同一个 Cloudflare 账户，zone 已激活。
...
4. 查看原主站和 `md-altas` 现有的 Workers Routes / Custom Domains，记录截图，确认下面两个路径没有需要保留的其他用途。
5. **不要新增 `reiinoki.dpdns.org` 的 Custom Domain 绑定给 `md-altas`；不要设置 `reiinoki.dpdns.org/*`。**
...
若需要在控制台核对或添加：
**Workers 和 Pages → md-altas → Settings → Domains & Routes → Add → Route**，选择正确的 zone。
...
Cloudflare Route 会匹配包含查询字符串的完整 URL。精确路由 `/md-altas` 可以覆盖无查询字符串的入口，
但 `/md-altas?from=bookmark` 不匹配它；已有斜杠的 `/md-altas/?from=bookmark` 则匹配 `/md-altas/*`。

**不要改成 `/md-altas*`**：它还会匹配 `/md-altas-other`、`/md-altas2` 等主站路径。
...
```text
(http.host eq "reiinoki.dpdns.org" and http.request.uri.path eq "/md-altas")
```

- 跳转类型：Static（静态）
- 目标：`https://reiinoki.dpdns.org/md-altas/`
...
先检查 Workers 默认预览地址的 `/md-altas/`，再检查正式域名：

- `/md-altas/`：四视图、地图 Worker、语言切换、详情、窄屏布局。
- `/md-altas/data/archive.json`：返回 JSON，而不是 SPA HTML。
- `/md-altas/city-name-credits.html`：规范化跳转留在 `/md-altas/` 内，返回链接仍指向 MD Atlas。
- `/md-altas`、`/md-altas?from=test`：跳转到带斜杠的入口，保留查询参数。
